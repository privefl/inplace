...
#define ON_VEC(RTYPE, CTYPE) {                                                 \
  substract_vec_mult< Vector<RTYPE>, RTYPE >(x, val);                           \
  break;                                                                       \
}
...
#define ON_SUB_VEC(RTYPE, CTYPE) {                                             \
  ...
  substract_vec_mult(VecAcc<RTYPE, CTYPE>(x2, i), val2);                        \
  break;                                                                       \
}
...
#define ON_SUB_MAT(RTYPE, CTYPE) {                                             \
  Matrix<RTYPE> x2(x);                                                         \
  Vector<RTYPE> val2(val);                                                     \
  substract_mat_mult(MatAcc<RTYPE, CTYPE>(x2, i, j), val2);                     \
  break;                                                                       \
}
...
void substract_vec_mult(C_VEC x, const Vector<RTYPE>& val) {
...
void substract_mat_mult(MatAcc<RTYPE, CTYPE> x, const Vector<RTYPE>& val) {
...
// [[Rcpp::export]]
void substract_submat_mult(SEXP x,
