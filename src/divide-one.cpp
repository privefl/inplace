...
#define ON_VEC(RTYPE, CTYPE) {                                                 \
  divide_vec_one< Vector<RTYPE>, CTYPE >(x, as<CTYPE>(val));                 \
  break;                                                                       \
}

#define ON_SUB_VEC(RTYPE, CTYPE) {                                             \
  Vector<RTYPE> x2(x);                                                         \
  divide_vec_one(VecAcc<RTYPE, CTYPE>(x2, i), as<CTYPE>(val));               \
  break;                                                                       \
}

#define ON_SUB_MAT(RTYPE, CTYPE) {                                             \
  Matrix<RTYPE> x2(x);                                                         \
  divide_mat_one(MatAcc<RTYPE, CTYPE>(x2, i, j), as<CTYPE>(val));            \
  break;                                                                       \
}
...
void divide_vec_one(C_VEC x, CTYPE val) {
...
void divide_mat_one(MatAcc<RTYPE, CTYPE> x, CTYPE val) {
